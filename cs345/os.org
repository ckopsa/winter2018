...
* TODO Homework 5 [0/7]
  DEADLINE: <2018-02-07 Wed>
** TODO (3) What code is responsible for doing context switches between the user-level threads in a process? What is this code required to do?

** TODO (3) Explain how a multithreaded application may provide better performance than a single-threaded application even on a single-processor system.

** TODO (2) Which of the following components of program state are shared across threads in a multithreaded process? (There may be more than one.)

   A) Register values	B) Heap Memory C) Global variables D) Stack memory

** TODO (3) Discuss three advantages of user-level threads (ULTs) over kernel-level threads (KLTs).

** TODO (1) What are the three main thread libraries in use today?

** TODO (3) List three threading issues. Briefly explain each of the issues (not how the issue might be solved, jut what is the issue).

** TODO (3) A Solaris thread which is currently running on a CPU may voluntarily give up the CPU to another thread. Which of the following statements is correct:
